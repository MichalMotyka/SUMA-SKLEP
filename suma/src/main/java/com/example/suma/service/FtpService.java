...
public class FtpService {
    ...
    public ImageEntity uploadFileToFtp(MultipartFile file) throws FtpConnectionException,IOException {
        try {
            ...
            String remoteFilePath = FTP_ORIGIN_DIRECTORY +"/"+ uuid+".png";
    ...
    public ByteArrayOutputStream getFile(String uuid) throws IOException {
        FTPClient ftpClient = getFtpConnection();
        ByteArrayOutputStream outputStream = new ByteArrayOutputStream();
        boolean downloaded = ftpClient.retrieveFile(uuid+".png",outputStream);
        ftpClient.logout();
        ftpClient.disconnect();
        if (downloaded){
            return outputStream;
